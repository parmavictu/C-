...
namespace CursoCSharp.TopicosAvancados {
    class LINK2 {
        public static void Executar() {
            ...
            var pedro = alunos.Single(aluno => aluno.Nome.Equals("Pedro"));//retorna só um que for igual
            Console.WriteLine($"{pedro.Nome} {pedro.Nota}");

            var fulano = alunos.SingleOrDefault(aluno => aluno.Nome.Equals("fulano"));//Esse em vez de dar erro ele retorna null qnd não acha
            if (fulano == null)
            {
            ...
            var exemploSkip = alunos.Skip(1).Take(3);//pula 1 pega 3
